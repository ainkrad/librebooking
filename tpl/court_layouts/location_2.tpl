<link rel="stylesheet" href="css/court-maps.css" type="text/css" />

<div style="text-align: center; padding: 10px;">
     <p style="color:#666; font-size:14px; margin-bottom:15px;">Tap a court on the venue map below to select it:</p>

    <div class="blueprint-wrapper">
        
        <img src="img/testcourt2.png" alt="Venue Layout" class="blueprint-image">

          <div class="map-court-overlay dynamic-court-btn"
                data-court-id="1"
                data-resource-id="1"
                data-resource-name="Court 1"
                style="
                    top: 35%;
                    left: 6%;
                    width: 7%;
                    height: 45%;">
               <span class="court-label">Court 1</span>
          </div>

          <div class="map-court-overlay dynamic-court-btn" 
                data-court-id="2"
                data-resource-id="2"
                data-resource-name="Court 2"
                style="
                    top: 35%;
                    left: 19%;
                    width: 7%;
                    height: 45%;">
               <span class="court-label">Court 2</span>
          </div>

          <div class="map-court-overlay dynamic-court-btn" 
                data-court-id="3"
                data-resource-id="3"
                data-resource-name="Court 3"
                style="
                    top: 35%;
                    left: 32%;
                    width: 7%;
                    height: 45%;">
               <span class="court-label">Court 3</span>
          </div>

          <div class="map-court-overlay dynamic-court-btn" 
                data-court-id="4"
                data-resource-id="4"
                data-resource-name="Court 4"
                style="
                    top: 35%;
                    left: 54%;
                    width: 7%;
                    height: 45%;">
               <span class="court-label">Court 4</span>
          </div>

          <div class="map-court-overlay dynamic-court-btn" 
                data-court-id="5"
                data-resource-id="5"
                data-resource-name="Court 5"
                style="
                    top: 35%;
                    left: 67%;
                    width: 7%;
                    height: 45%;">
               <span class="court-label">Court 5</span>
          </div>

          <div class="map-court-overlay dynamic-court-btn" 
                data-court-id="6"
                data-resource-id="6"
                data-resource-name="Court 6"
                style="
                    top: 35%;
                    left: 81%;
                    width: 7%;
                    height: 45%;">
               <span class="court-label">Court 6</span>
          </div>

    

        {* <div class="map-court-overlay dynamic-court-btn" 
             data-court-id="3" 
             style="top: 65%; left: 10%; width: 50%; height: 25%;">
        </div> *}

    </div>
</div>

{literal}
<script>
document.addEventListener('DOMContentLoaded', function() {
     var resourceNameMap = {};
{/literal}
{foreach from=$AvailableResources item=resource}
     resourceNameMap[{$resource->Id}] = "{$resource->Name|escape:'javascript'}";
{/foreach}
{literal}
     document.querySelectorAll('.map-court-overlay').forEach(function(el) {
          var rid = el.getAttribute('data-resource-id');
          var label = el.querySelector('.court-label');
          if (!label) return;
          if (rid && resourceNameMap[rid]) {
               label.textContent = resourceNameMap[rid];
          } else {
               // fallback to provided static name or keep existing text
               label.textContent = el.getAttribute('data-resource-name') || label.textContent;
          }
     });
});
</script>
{/literal}